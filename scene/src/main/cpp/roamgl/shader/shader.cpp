...
Shader::Shader(AAssetManager *assetManager, const std::string &pathToInternalDir,
    ...
    unsigned int vs = compileShader(GL_VERTEX_SHADER,
                                    parseShader(assetManager, pathToInternalDir, vertexPath));
    unsigned int fs = compileShader(GL_FRAGMENT_SHADER,
                                    parseShader(assetManager, pathToInternalDir, fragmentPath));
...
std::string
Shader::parseShader(AAssetManager *assetManager, const std::string &pathToInternalDir,
                    const std::string &assetFilepath) {
    std::string fullPath = pathToInternalDir + "/" + FileUtils::GetFileName(assetFilepath);
    bool isFilePresent = FileUtils::ExtractAssetReturnFilename(assetManager, fullPath,
                                                               assetFilepath);
    if (!isFilePresent) {
        return "";
    }
