...
LineBucket::LineBucket(const float zoom_,
                       const uint32_t overscaling_)
                       : zoom(zoom_), overscaling(overscaling_) {
...
void LineBucket::addGeometry(const GeometryCoordinates& coordinates) {
    // todo
    const FeatureType type = FeatureType::LineString;
    ...
    // todo
    const LineCapType beginCap = LineCapType::Round;
    const LineCapType endCap = type == FeatureType::Polygon ? LineCapType::Butt : LineCapType::Round;
