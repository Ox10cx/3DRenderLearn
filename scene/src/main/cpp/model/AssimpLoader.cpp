...
AssimpLoader::AssimpLoader() {
    m_pImporterPtr = new Assimp::Importer;
    m_pAiScene = NULL;
    ...
    m_nShaderProgramID         = LoadShaders(vertexShader, fragmentShader);
    m_nVertexAttribute         = GetAttributeLocation(m_nShaderProgramID, "vertexPosition");
    m_nVertexUVAttribute       = GetAttributeLocation(m_nShaderProgramID, "vertexUV");
    m_MvpLocation             = GetUniformLocation(m_nShaderProgramID, "mvpMat");
    m_TextureSamplerLocation  = GetUniformLocation(m_nShaderProgramID, "textureSampler");
...
AssimpLoader::~AssimpLoader() {
    Delete3DModel();
    if(m_pImporterPtr) {
        delete m_pImporterPtr;
        m_pImporterPtr = NULL;
    }
    m_pAiScene = NULL; // gets deleted along with importerPtr
}
...
void AssimpLoader::GenerateGLBuffers() {
    ...
    for (unsigned int n = 0; n < m_pAiScene->mNumMeshes; ++n) {

        const aiMesh *mesh = m_pAiScene->mMeshes[n]; // read the n-th mesh
        ...
        newMeshInfo.m_nNumberOfFaces = m_pAiScene->mMeshes[n]->mNumFaces;

        // buffer for faces
        if (newMeshInfo.m_nNumberOfFaces) {
            ...
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                         ...
                         GL_STATIC_DRAW);
            newMeshInfo.m_nFaceBuffer = buffer;
        ...
        if (mesh->HasPositions()) {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                         ...
                         GL_STATIC_DRAW);
            newMeshInfo.m_nVertexBuffer = buffer;
        ...
        if (mesh->HasTextureCoords(0)) {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                         ...
                         GL_STATIC_DRAW);
            newMeshInfo.m_nTextureCoordBuffer = buffer;
        ...
        aiMaterial *mtl = m_pAiScene->mMaterials[mesh->mMaterialIndex];
        aiString texturePath;	//contains filename of texture
        if (AI_SUCCESS == mtl->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath)) {
            unsigned int textureId = mTextureNameMap[texturePath.data];
            newMeshInfo.m_nTextureIndex = textureId;
        } else {
            newMeshInfo.m_nTextureIndex = 0;
        }

        mModelMeshes.push_back(newMeshInfo);
    }
}
 ...
 */
bool AssimpLoader::LoadTexturesToGL(const std::string& modelFilename) {
    ...
    for (unsigned int m = 0; m < m_pAiScene->mNumMaterials; ++m) {
        ...
        aiReturn isTexturePresent = m_pAiScene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE,
                                                                     textureIndex,
                                                                     &textureFilename);
        while (isTexturePresent == AI_SUCCESS) {
            //fill map with textures, OpenGL image ids set to 0
            mTextureNameMap.insert(std::pair<std::string, GLuint>(textureFilename.data, 0));

            // more textures? more than one texture could be associated with a material
            textureIndex++;
            isTexturePresent = m_pAiScene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE,
                                                                textureIndex, &textureFilename);
        }
    }

    int numTextures = (int) mTextureNameMap.size();
    ...
    std::map<std::string, GLuint>::iterator textureIterator = mTextureNameMap.begin();
    int i = 0;
    for (; textureIterator != mTextureNameMap.end(); ++i, ++textureIterator) {
...
bool AssimpLoader::Load3DModel(const std::string& modelFilename) {
    ...
    m_pAiScene = m_pImporterPtr->ReadFile(modelFilename, aiProcessPreset_TargetRealtime_Quality);

    // Check if import failed
    if (!m_pAiScene) {
        std::string errorString = m_pImporterPtr->GetErrorString();
...
void AssimpLoader::Render3DModel(glm::mat4 *mvpMat) {
    ...
    glUniformMatrix4fv(m_MvpLocation, 1, GL_FALSE, (const GLfloat *) mvpMat);

    glActiveTexture(GL_TEXTURE0);
    glUniform1i(m_TextureSamplerLocation, 0);

    unsigned int numberOfLoadedMeshes = mModelMeshes.size();

    // render all meshes
    for (unsigned int n = 0; n < numberOfLoadedMeshes; ++n) {

        // Texture
        if (mModelMeshes[n].m_nTextureIndex) {
            glBindTexture( GL_TEXTURE_2D, mModelMeshes[n].m_nTextureIndex);
        }

        // Faces
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mModelMeshes[n].m_nFaceBuffer);

        // Vertices
        glBindBuffer(GL_ARRAY_BUFFER, mModelMeshes[n].m_nVertexBuffer);
        glEnableVertexAttribArray(m_nVertexAttribute);
        glVertexAttribPointer(m_nVertexAttribute, 3, GL_FLOAT, 0, 0, 0);

        // Texture coords
        glBindBuffer(GL_ARRAY_BUFFER, mModelMeshes[n].m_nTextureCoordBuffer);
        glEnableVertexAttribArray(m_nVertexUVAttribute);
        glVertexAttribPointer(m_nVertexUVAttribute, 2, GL_FLOAT, 0, 0, 0);

        glDrawElements(GL_TRIANGLES, mModelMeshes[n].m_nNumberOfFaces * 3, GL_UNSIGNED_INT, 0);
